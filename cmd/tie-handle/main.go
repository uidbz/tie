...
func main() {
	...
	if *settingsPtr != "" {
		...
		err := getlib.DownloadFile(nil, app.url, app.inputHash, app.settings, nil)
		if err != nil {
	...
	if *processDirPtr {
		err := getlib.DownloadFile(nil, app.url, app.inputHash, app.tmpDir, nil)
		if err != nil {
			app.LogOutput("Error downloading input: " + err.Error())
		}
		app.Process(app.tmpDir)
	} else {
		getlib.ExecForEach(nil, app.url, app.inputHash, &app, "", nil)
	}
