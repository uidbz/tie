...
Copy `tie-triplestore.toml.example` to `tie-triplestore.toml` and edit it. Access
management is done by adding/removing `[[Users]]` entries; passwords are stored
in plaintext, so keep the file readable only by the server's user.

### Installation & run
```
git clone https://github.com/uidbz/tie
cd tie
go build ./cmd/tie-triplestore
./tie-triplestore -config tie-triplestore.toml
```

### Run with Docker

The `Dockerfile` at the repo root builds a single image that runs both
tie-triplestore and tie-filehost together:
```
docker build -t tie .
