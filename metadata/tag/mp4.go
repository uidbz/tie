...
func (m metadataMP4) readAtoms(r io.ReadSeeker) error {
	for {
		...
		case "mvhd":
			if err := m.readMovieHeader(r, size); err != nil {
				return err
			}
			continue
		}
...
// readMovieHeader parses the mvhd atom for the movie timescale and duration,
// recording the playing time in seconds. size is the full atom size including
// its 8-byte header. The layout is a 1-byte version, 3 flag bytes, then either
// 32-bit (version 0) or 64-bit (version 1) creation/modification times, a 32-bit
// timescale, and a duration of the matching width.
func (m metadataMP4) readMovieHeader(r io.ReadSeeker, size uint32) error {
	if size < 8 {
		return errors.New("invalid mvhd size")
	}
	...
	if timescale > 0 && duration > 0 {
		...
		m.data["_duration"] = time.Duration(secs * float64(time.Second))
	}
	return nil
}

func (m metadataMP4) Duration() time.Duration {
	d, _ := m.data["_duration"].(time.Duration)
	return d
}
