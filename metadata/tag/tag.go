...
//
// This is a fork of github.com/dhowden/tag (BSD-licensed, see LICENSE) vendored
// into tie and extended with audio-duration extraction: the Metadata interface
// gains a Duration() method, computed from the container/stream headers for
// MP3 (Xing/Info/VBRI or CBR frame parse), FLAC (STREAMINFO), Ogg Opus/Vorbis
// (final granule position), and MP4 (mvhd). Formats where duration is not
// derived report a zero Duration.
package tag
...
type Metadata interface {
	...
	// Duration returns the playing time of the audio stream, derived from the
	// container/stream headers, or zero when it cannot be determined.
