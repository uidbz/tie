...
func ReadID3v2Tags(r io.ReadSeeker) (Metadata, error) {
	...
	// Audio frames begin just past the tag: the 10-byte header plus the tag body
	// size (which already includes any extended header and padding). DSF reuses
	// this reader for an ID3 chunk that is not followed by MPEG frames; its
	// wrapper overrides Duration, so the value computed here is harmless there.
	return metadataID3v2{header: h, frames: f, duration: mp3Duration(r, 10+int64(h.Size))}, nil
}
